                    ...
                    <div class="pt-1 flex-grow">
                        <div class="flex items-center justify-center">
                            ...
                            <div class="flex-1 my-1">
                                <a href="{{ route('user.profile', $post->user_id) }}" class="hover:underline text-blue-800">{{ App\User::find($post->user_id)->first_name }} {{ App\User::find($post->user_id)->last_name }}</a>
                        ...
                        <article class="pt-3 pb-5 text-grey-darkest">
                            {{ $post->content }}
                            @if($post->media !== null)
                                <img class="h-64" src="{{ asset('storage/'.$post->media) }}" alt="">
                            @endif
                        </article>
...
@empty
    <div class="w-11/12 mx-auto">
        <div class="alert alert-info">
            <div class="text-xl font-bold">Malheureusement, votre fil d'actualité est vide :(</div>
            <p class="mt-1 text-base">Que diriez-vous de publier <a class="link" href="{{ route('post.create') }}">votre premier post</a> ou de commencer à <a class="link" href="{{ route('discover') }}">suivre des personnes</a>?</p>
        </div>
    </div>
@endforelse
